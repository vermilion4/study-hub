// Inline Lucide icons (https://lucide.dev) — used both in .astro (via set:html)
// and in client scripts (as strings). Kept as raw SVG so there is no runtime
...
export function typeIcon(type: string, size = 18): string {
  return icon(TYPE_ICON[type] ?? 'file-text', size);
}
